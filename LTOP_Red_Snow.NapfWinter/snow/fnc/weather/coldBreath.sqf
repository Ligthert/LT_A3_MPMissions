...
if (_unit getVariable ["IP_ColdBreath", false]) exitWith {["Already running!"] call BIS_fnc_error};
_unit setVariable ["IP_ColdBreath", true];
if (!(_unit getVariable ["IP_MonitorLifeSignsRunning", false])) then {[_unit] call IP_fnc_monitorLifeSigns};
...
_handle = [_unit, _condition] spawn {
	...
	while {(alive _unit) && (call _condition)} do {
		...
		waitUntil {!(_unit getVariable ["IP_SimpleSentence_Talking", false])};
		sleep 0.5;
		
		deleteVehicle _source;
		deleteVehicle _fog;
		
		_breathing = _unit getVariable ["IP_Breathing", 15];
		_delay = time + (60 / _breathing);
		waitUntil {((time > _delay) OR (_unit getVariable ["IP_SimpleSentence_Talking", false])) && {(vehicle _unit == _unit) && (simulationEnabled _unit)}};
	};
	
	_unit setVariable ["IP_ColdBreath", false];
};
