...
while {1 == 1} do {
        ...
        if ( paramsArray select 0 == 1 ) then {
                ...
                if (alive _unit) then {
                        ...
                        if ( _nearPlayer == 0 ) then {
                                ...
                                // DEBUG
                                // if ( paramsArray select 0 == 1 and !isNil _marker) then { deleteMarker _marker; };
                        ...
                        // Deletion was to fast.. we need time to loot
                        // deleteVehicle _unit;

                        // Delete the Marker once dead -- BUGS out for some reason
                        // if ( paramsArray select 0 == 1 ) then { deleteMarker _marker; };
        ...
        diag_log format["%4 caching/spawnin-system -- ALiVE: %1; Cached: %2; Total: %3;",count alive_units, count _cached, count alive_units + count _cached, time];
